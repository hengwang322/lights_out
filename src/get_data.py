...
def clean(df):
    df_clean = df.copy()
    df_clean['wattage']=df_clean['wattage'].apply(lambda x: np.nan if x == 0 else x)
    df_clean['lamp_type']=df_clean['lamp_type'].apply(lambda x: np.nan if x == ' ' else x)
    df_clean = df_clean.dropna()
    
    important_lamp_type = ['MV','CFL','HPS','LED']
    
    df_clean['lamp_type']=df_clean['lamp_type'].apply(lambda x:'OTHER' if x not in important_lamp_type else x)
...
def get_light_data(LIGHT_GEOJSON):
    ...
    light = light[cols]
    col_name = ['street_name', 'street_type', 'lamp_type', 'longitude', 'latitude', 'wattage']
